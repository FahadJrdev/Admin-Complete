...
import { InvestorDatosItem, InvestorItem , FundItem, ProjectItem, RiskItem } from './InvestorTable';
...
export const RiskTable = ({header1,header2,header3,data,language}) => {
    const [tableItems, setTableItems] = useState([]);
    const [loading, setLoading] = useState(false);
    const [itemsPerPage] = useState(5);
  const [currentPage, setCurrentPage] = useState(1);
        ...
        <div className="table-responsive">
            <div className="table-lists projectTable riskTable">
                ...
                </ul>
                {
                    currentItems.map((info,i)=>{
                        return(
                            <RiskItem key={i} info={info} language={language} />
                        )
                    })
                }
            </div>
