...
const Projects = ({lang, setLang, language, responsive}) => {
  ...
  const [projectStatus, setProjectStatus] = useState('close');
  ...
  return (
    <>
      ...
      {
        projectStatus === 'open'
        ?<ProjectStatus language={language} title={language.projects.status} status={status} setStatus={setStatus} status1={`Start`} status2={`Implementation`} status3={`Monitoring`} status4={`Disinvestment`} />
        :<></>
      }
